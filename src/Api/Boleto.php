...
class Boleto implements ParserInteface
{
     ...
     * @return array
     */
    public function getInstrucaoArray(): array
    {
        return $this->instrucaoArr;
    }
    ...
    {
        $beneficiario = (!empty($this->beneficiario)) ? $this->getBeneficiario()->parserCarne() : [];

        return array_merge_recursive([
            'emissao'    => $this->emissao->format('Y-m-d'),
            'vencimento' => $this->vencimento->format('Y-m-d'),
            'documento'  => $this->documento,
            'numero'     => $this->numero,
            'titulo'     => $this->titulo,
            'valor'      => number_format($this->valor, 2, '.', ''),
            'instrucao' => $this->getInstrucaoArray(),
        ], $this->getConta()->parserCarne(), $beneficiario, $this->getPagador()->parserCarne());
    }
}
